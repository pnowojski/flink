...
public abstract class KafkaMigrationTestBase extends KafkaTestBase {
	...
	protected static String TOPIC = "flink-kafka-producer-migration-test";
	...
	public String getKafkaDirectory() {
		return getKafkaDirectory(testMigrateVersion);
	}

	public String getKafkaDirectory(MigrationVersion version) {
		return "src/test/resources/kafka-migration-kafka-dir-flink-" + version;
	}

	public String getZooKeeperDirectory() {
		return getZooKeeperDirectory(testMigrateVersion);
	}

	public String getZooKeeperDirectory(MigrationVersion version) {
		return "src/test/resources/kafka-migration-zookeeper-dir-flink-" + version;
	}
	...
	/**
	 * startClusters with predefined/pre-initiated directories.
	 */
	protected static void startClusters(String zooKeeperDirectory, String kafkaDirectory) throws Exception {
		startClusters(KafkaTestEnvironment.createConfig()
			.setKafkaServersNumber(NUMBER_OF_KAFKA_SERVERS)
			.setZooKeeperDirectory(zooKeeperDirectory)
			.setKafkaDirectory(kafkaDirectory));
	}
	...
	public void writeSnapshot() throws Exception {
		try {
			...
			kafkaServer.copyZooKeeperDirectory(getZooKeeperDirectory(flinkGenerateSavepointVersion.get()));
			kafkaServer.copyKafkaDirectory(getKafkaDirectory(flinkGenerateSavepointVersion.get()));
		}
		finally {
			shutdownClusters();
		}
	...
	public void testRestoreProducer() throws Exception {
		try {
			startClusters(getZooKeeperDirectory(), getKafkaDirectory());
			try (OneInputStreamOperatorTestHarness testHarness = createTestHarness()) {
