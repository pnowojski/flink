...
public class FileBasedBufferIterator implements CloseableIterator<Buffer> {
	...
	public Buffer next() {
		...
		NetworkBuffer next = new NetworkBuffer(wrap(buffer), FreeingBufferRecycler.INSTANCE, DATA_BUFFER, bytesRead);
		lastBuffer = next;
		return next;
	}
	...
	public void close() throws Exception {
		closeAll(stream, file::release, () -> lastBuffer.recycleBuffer());
	}
}
