...
public class FinishingValidator implements TestOperatorLifecycleValidator {
    ...
    public void validateOperatorLifecycle(
        ...
        long finalCheckpointID = -1;
        boolean finalCheckpointFinished = false;
        for (TestEvent ev : operatorEvents) {
            if (ev instanceof OperatorFinishedEvent) {
                opFinished = true;
            } else if (ev instanceof CheckpointStartedEvent) {
                if (opFinished && !finalCheckpointFinished) {
                    finalCheckpointID = ((CheckpointStartedEvent) ev).checkpointID;
                }
            } else if (ev instanceof CheckpointCompletedEvent) {
                if (finalCheckpointID == ((CheckpointCompletedEvent) ev).checkpointID) {
                    finalCheckpointFinished = true;
                }
            } else if (opFinished) {
                fail(
                        format(
        ...
        assertTrue(
                format(
                        ...
                        operatorId, subtaskIndex, job.eventQueue.getAll()),
                opFinished);
        assertTrue(
                format(
                        "Operator %s[%d] was finished but didn't finish the checkpoint after that;"
                                + "last checkpoint started after finish: %d (events (excluding watermarks): %s)",
                        operatorId,
                        subtaskIndex,
                        finalCheckpointID,
                        job.eventQueue.getAll().stream()
                                .filter(ev -> !(ev instanceof WatermarkReceivedEvent))
                                .collect(toList())),
                finalCheckpointFinished);
    }
}
